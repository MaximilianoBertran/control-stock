...
@isset($roles)
    <div class="form-group row">
        <label for="email" class="col-md-4 col-form-label text-md-right">@lang('Roles')</label>
        <div class="col-md-6">
            {!! Form::select('roles[]', $roles, isset($rolesSelected) ? $rolesSelected : null, array('class' => 'custom-select select2-select', 'id' => 'roles', 'multiple' => 'multiple')) !!}
            @error('roles')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
